...
- **`tenant_id`** is not yet designed into the schema, despite the SaaS direction — should be
  settled before per-pipeline ClickHouse tables are finalized, since retrofitting it after
  tables exist is more expensive.
