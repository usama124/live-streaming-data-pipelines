...
- ~~**`tenant_id`** is not yet designed into the schema~~ — **Resolved.** Live pipelines use
  the same `ch_unique_identifier` scheme normal pipelines already use:
  `user_<user_id>_collection_<collection_number>_<table_name>` (e.g.
  `user_1_collection_22_aveva_iot`), generated by the same shared logic both paths call. No
  separate `tenant_id` concept exists in this system — see `ARCHITECTURE.md` §4.
